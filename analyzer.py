            ...
            if ip_src == device_ipv4 or ip_dst == device_ipv4 or ip_src == device_ipv6 or ip_dst == device_ipv6 or ip_dst == BROADCAST or ip_dst == BROADCAST_IPV6 or ip_dst == SSDP or phone_condition:
                ...
                if packet.haslayer(HTTP):
                    try:
                        ...
                        pass
                # Check DNS case
                elif packet.haslayer(DNS) and not ((my_node_1 is not None and my_node_1.protocol == "udp" and my_node_1.element.dst_port == 5353) or packet[DNS].qd.qname.decode().endswith(".local.")):
                    try:
                        type_name = None
                        ...
                        pass
                # Check mDNS case
                if packet.haslayer(DNS) and ((my_node_1 is not None and my_node_1.protocol == "udp" and my_node_1.element.dst_port == 5353) or packet[DNS].qd.qname.decode().endswith(".local.")):
                    try:
